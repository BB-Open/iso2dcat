...
class RdfValidator2(BaseDCM):

    def __init__(self):
        super(RdfValidator2, self).__init__()
        self.setup_components()
    ...
    def run(self, db_name=None, limit=None):
        ...
        self.logger.info(len(datasets))
        start = time.time()
        self.get_publisher(db_name, datasets)
        self.get_distributions(db_name, datasets)
        end = time.time()
        self.logger.info(f'Time needed {end-start}s')
        self.logger.info('rdf datasets loaded')
    ...
    def get_distributions(self, db_name, datasets):
        if db_name is None:
            db_name = self.cfg.RDF2SOLR_SOURCE
        values = '<' + '> <'.join(datasets) + '>'
        results = self.rdf4j.query_repository(db_name, DISTRIBUTIONS_FOR_ALL_DATASET.format(values=values))
        triples = results['results']['bindings']
        dist_uris = []
        # For all datasets collect all attributes
        for res in triples:
    ...
    def get_publisher(self, db_name, datasets):
        if db_name is None:
            db_name = self.cfg.RDF2SOLR_SOURCE
        values = '<' + '> <'.join(datasets) + '>'
        results = self.rdf4j.query_repository(db_name, PUBLISHER_FOR_ALL_DATASETS.format(values=values))
        triples = results['results']['bindings']
        publisher_uri = []
        # For all datasets collect all attributes
        for res in triples:
